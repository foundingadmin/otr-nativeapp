...
// `sub` (DR-CPC-002): an optional one-line supporting descriptor shown under
// the headline. Present only on money / legal-gate / high-stakes items that
// need the who / why / stakes spelled out; simple info and positive notices
// stay a bare headline.
function cardAlert(c){
  const firm = c.firm && c.firm !== '—' ? c.firm : 'your attorney';
  if (c.balance && c.balance.overdue)
    return { tone:'error', faIcon:'fa-circle-exclamation', text:'Payment overdue', value:`$${c.balance.amount}`, cta:'Make a payment',
      sub:`Clear the balance to keep ${firm} working your case.` };
  ...
  if (c.status === 'deadline')
    return { tone:'error', faIcon:'fa-triangle-exclamation', text:`Only ${c.daysToCourt} days left to book`, cta:'Complete booking',
      sub:`Book an attorney before your ${c.courtDate} court date.` };
  if (c.status === 'missed')
    return { tone:'error', faIcon:'fa-triangle-exclamation', text:'Potential risk for active warrant', cta:'Contact us now',
      sub:`You missed your ${c.courtDate} court date. Contact us before a warrant issues.` };
  if (c.status === 'counter')
    return { tone:'warning', faIcon:'fa-scale-balanced', text:'Your attorney sent a counter offer', value:`$${c.counter.to}`, cta:'Review counter offer',
      sub:`${firm} raised the quote from $${c.counter.from}. Review the new terms before you respond.` };
  ...
  if (c.status === 'active-action')
    return { tone:'warning', faIcon:'fa-file-signature', text:'Sign your letter of engagement', cta:'Sign & continue',
      sub:`Confirms ${firm} to represent you at your ${c.courtDate} court date.` };
  if (c.status === 'active' && c.courtSoon)
    return { tone:'warning', faIcon:'fa-gavel', text:`Court date in ${c.courtSoon} days, let's prep`, cta:'Review checklist',
      sub:`We'll build your prep checklist so you walk in ready.` };
...
// The grouped alert block — the persistent home for every card toast.
// DR-CPC-002 (decided in exploration 02-alert-cta-sizing): the block itself is
// the tap target. There is NO separate button; the message, an optional
// one-line supporting descriptor (`al.sub`), and a trailing edge chevron ARE
// the affordance. Tapping anywhere in the tinted block resolves the alert.
//   · headline only (V4)     — signals that read fine unaided (unread messages)
//   · headline + sub (V4b/c/d)— money / legal-gate / high-stakes items that need
//     the who / why / stakes spelled out (LOE, counter offer, payment overdue…)
function GroupedAlert({ al }){
  const t = ALERT_TONE[al.tone];
  const tall = !!(al.sub || al.profile); // multi-line toast → chevron drops to the bottom-right corner; one-liners keep it centered
  return (
    <div style={{ position:'relative', marginTop:12, background:t.bg, border:`1px solid ${t.line}`, borderRadius:14, padding:'12px 34px 12px 12px' }}>
      {al.profile && (
        <div style={{ paddingBottom:11, marginBottom:11, borderBottom:`1px solid ${t.line}` }}>
          <AttorneyRow c={al.profile} tint={{ primary:t.ink, secondary:t.icon }} />
        </div>
      )}
      <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {al.sub && <div style={{ marginTop:3, fontSize:12, fontWeight:500, color:t.ink, opacity:.72, lineHeight:1.4, textWrap:'pretty' }}>{al.sub}</div>}
        </div>
      </div>
      <div style={{ position:'absolute', right:13, display:'flex', ...(tall ? { bottom:12 } : { top:'50%', transform:'translateY(-50%)' }) }}>
        <FI name="chevron-right" size={16} color={t.icon} />
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginTop:12, background:'var(--green-25)', border:'1px solid var(--green-100)', borderRadius:14, padding:12 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:13.5, fontWeight:700, color:'var(--green-900)' }}>{c.firm}</div>
...
// The locked-in CPC header (identity zone) — DR-CPC-001, decided from
// exploration 01 (header-divergences, take D2): an "Issued · month year"
// eyebrow over the location headline, the lifecycle badge, and a single
// hollow violation pill (charge name + overflow count for multi-charge
// cases) in place of the old filled classification tag + separate court-date
// line. CPCConsistent and every compact tile elsewhere render through THIS,
// so a change here propagates to every CPC surface at once.
function CPCHeader({ c }){
  const n = c.more || 0;
  const viol = window.tViol(c) || window.clsLabel(c);
  return (
    <>
      <div style={{ display:'flex', justifyContent:'space-between', gap:8, alignItems:'flex-start' }}>
        <div style={{ minWidth:0 }}>
          <div className="micro" style={{ whiteSpace:'nowrap' }}>Issued · {window.tDate(c)}</div>
          <p className="ttl" style={{ marginTop:1 }}>{window.tPlace(c)}</p>
        </div>
        <StatusBadge status={window.badgeStatus(c.status)} note={null} />
      </div>
      <div style={{ display:'flex', gap:6, marginTop:10, alignItems:'center', flexWrap:'wrap' }}>
        <span className="pill-outline">{viol}{n > 0 ? ` +${n}` : ''}</span>
      </div>
    </>
  );
}
...
function CPCConsistent({ c }){
  ...
  // V0 (DR-CPC-002): a card with no action signal shows NO toast and NO button.
  // The whole card is the tap target, carrying one edge chevron kept clear of
  // the top-right badge. Only the concluded wraps and the SmartMatch quote
  // block keep their own in-block CTAs.
  const tap = !(c.balance && c.balance.overdue) && c.status!=='dismissed' && c.status!=='resolved'
    && c.status!=='cancelled' && c.status!=='reassigned' && !showQ && !al;
  return (
    <div className={`cpc${unfinished ? ' unfinished' : ''}${concluded ? ' concluded' : ''}`}>
      ...
      {(c.balance && c.balance.overdue) ? <GroupedAlert al={al} />
        ...
        : null}
      {tap && <div style={{ position:'absolute', right:14, bottom:12, display:'flex' }}>
        <FI name="chevron-right" size={16} color="var(--wf-faint)" /></div>}
    </div>
  );
}
