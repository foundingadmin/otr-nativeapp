...
/* __IIFE__ */;(function () {
  ...
  function CaseFacts({ rec, inlineViolations = 1, completionPrompts = true }) {
    ...
    return (
      ...
      {rec.attorney &&
        <Fact ic="user-square-single" k="Attorney">
          <div className="f-v"><span className="trunc">{rec.attorney.name}</span>
            <div className="f-sub trunc">{rec.attorney.firm}</div></div>
        </Fact>
        }
  ...
  function TimelineZone({ rec }) {
    const ev = rec.timeline || [];
    if (!ev.length) return null;
    // DECISION (locked): the timeline rides directly on the primary surface —
    // no card, no "What's happened · what's next" header. The spine itself is
    // the reassurance anchor. The titled-card variant is preserved in
    // Archive/timeline-titled-card.jsx.
    return (
      <div className="tlbare">
      <Timeline rec={rec} />
    </div>);
