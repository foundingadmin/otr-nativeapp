...
function Attorney({ size = 38, blurred = false }) {
  ...
  return <div className={`wf-portrait${blurred ? ' blur' : ''}`} style={{ width: size, height: size, borderRadius: r }} />;
}

function Stars({ rating = 4.8, size = 12 }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, color: 'var(--amber-400,#fdb022)' }}>
      <FeatureIcon name="star-1" size={size} color="var(--amber-400,#fdb022)" />
