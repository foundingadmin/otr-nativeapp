...
// ─────────────────────────────────────────────────────────────
function DCSection({ id, title, subtitle, eyebrow, updated, children, gap = 48 }) {
  ...
  return (
    <div data-dc-section={sid}
      ...
      <div style={{ padding: '0 60px' }}>
        <div className="dc-sectionhead" style={{ paddingBottom: 36, maxWidth: 760 }}>
          {eyebrow && <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 1.4, textTransform: 'uppercase', color: 'rgba(60,50,40,0.5)', marginBottom: 9 }}>{eyebrow}</div>}
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, flexWrap: 'wrap' }}>
            <DCEditable tag="div" value={sec.title ?? title}
              onChange={(v) => ctx && sid && ctx.patchSection(sid, { title: v })}
              style={{ fontSize: 28, fontWeight: 600, color: DC.title, letterSpacing: -0.4, display: 'inline-block' }} />
            {updated && <span style={{ fontSize: 13.5, fontWeight: 500, color: 'rgba(60,50,40,0.55)', whiteSpace: 'nowrap' }}>{updated}</span>}
          </div>
          {subtitle && <div style={{ fontSize: 16, color: DC.subtitle, textWrap: 'pretty', marginTop: 8 }}>{subtitle}</div>}
        </div>
      </div>
