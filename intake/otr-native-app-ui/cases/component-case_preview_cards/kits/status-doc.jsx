...
const { DCSection, DCArtboard, CASES, StatusBadge, CPCConsistent, CPCHeader,
  GroupedAlert, ALERT_TONE, AttorneyRow } = window;
    ...
    <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 4px',
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:7, marginTop:4 }}>
          ...
          {tone && <>
            <i style={{ width:7, height:7, borderRadius:'50%', background:t.icon, flex:'none' }} />
            <span style={{ fontSize:11.5, color:'var(--wf-muted)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{text}</span>
          </>}
        </div>
      </div>
  ...
  return (
    <div style={{ display:'flex', gap:14, marginTop:18 }}>
      ...
      <Layer tag="Layer 2 • the action signal" color="var(--money)" title="What to do right now"
        body={<>The action signal. All urgency color lives here, not on the badge. It is a six tone block, and tapping the block resolves the action.</>} />
    </div>
  );
}
...
const TONES = [
  { tone:'error',    name:'Error • red',     use:'A money problem, like a payment overdue or failed',
    al:{ tone:'error', faIcon:'fa-circle-exclamation', text:'Payment overdue', value:'$109', cta:'Make a payment', sub:'Clear the balance to keep your attorney working your case.' } },
  { tone:'warning',  name:'Warning • amber',  use:'Something needs you, sign the letter of engagement, a court date, or a counter offer',
    al:{ tone:'warning', faIcon:'fa-file-signature', text:'Sign your letter of engagement', cta:'Sign & continue', sub:'Confirms your firm to represent you at your court date.' } },
...
function AlertSystem(){
  return (
    <Panel>
      ...
      <Lead>Every card alert uses the same grouped block. The alert and the affordance that resolves it share one tinted container: the whole block is the tap target, so no separate button is needed. Each tone has a fixed meaning, never a one off.</Lead>
      ...
      <div style={{ marginTop:30, paddingTop:26, borderTop:'1px solid var(--wf-line)' }}>
        ...
        <SurfaceRow n="1" title="Card signal" desc="The full grouped alert at the foot of the card. The whole tinted block is the tap target.">
          <GroupedAlert al={{ tone:'warning', faIcon:'fa-file-signature', text:'Sign your letter of engagement', cta:'Sign & continue', sub:'Confirms your firm to represent you at your court date.' }} />
        </SurfaceRow>
...
const PAIRINGS = {
  incomplete: { name:'Incomplete', note:'A red lifecycle badge with, at most, a red urgency toast.', rows:[
    ...
    { tone:'error', text:'Only 9 days left to book',      al:{ tone:'error', faIcon:'fa-triangle-exclamation', text:'Only 9 days left to book', cta:'Complete booking', sub:'Book an attorney before your court date passes.' } },
    { tone:'error', text:'Potential active warrant risk', al:{ tone:'error', faIcon:'fa-triangle-exclamation', text:'Potential risk for active warrant', cta:'Contact us now', sub:'You may have missed a court date. Contact us before a warrant issues.' } },
  ]},
  active: { name:'Active', note:'The big tent. A calm blue badge that pairs with the full tone range.', rows:[
    ...
    { tone:'warning', text:'Sign your letter of engagement', al:{ tone:'warning', faIcon:'fa-file-signature', text:'Sign your letter of engagement', cta:'Sign & continue', sub:'Confirms your firm to represent you at your court date.' } },
    { tone:'warning', text:'Counter offer · $189',           al:{ tone:'warning', faIcon:'fa-scale-balanced', text:'Your attorney sent a counter offer', value:'$189', cta:'Review counter offer', sub:'Reyes Traffic Law raised the quote from $149. Review the new terms before you respond.' } },
    ...
    { tone:'error',   text:'Payment overdue · $109',         al:{ tone:'error', faIcon:'fa-circle-exclamation', text:'Payment overdue', value:'$109', cta:'Make a payment', sub:'Clear the balance to keep your attorney working your case.' } },
  ]},
  resolved: { name:'Resolved', note:'A green badge with informational tones, plus an overdue balance.', rows:[
  ...
  dismissed: { name:'Dismissed!', note:'The win, a green badge with confetti, usually a review prompt.', rows:[
    ...
    { tone:'success', text:'Review your attorney',    al:{ tone:'success', faIcon:'fa-star', text:'Review your attorney', cta:'Leave a review', profile:{ firm:'Okonkwo Legal', attorney:{ rating:5.0 } } } },
    { tone:'info',    text:'2 unread messages',       al:{ tone:'info', faIcon:'fa-comment-dots', text:'2 unread messages', cta:'View updates' } },
    { tone:'error',   text:'Payment overdue · $89',   al:{ tone:'error', faIcon:'fa-circle-exclamation', text:'Payment overdue', value:'$89', cta:'Make a payment', sub:'Clear the balance to keep your attorney working your case.' } },
  ]},
  cancelled: { name:'Cancelled', note:'A gray terminal badge, at most a warrant risk red, an info note, or the money-back guarantee payout.', rows:[
    { tone:null,   text:'Case cancelled',                al:null },
    { tone:'error',text:'Potential active warrant risk', al:{ tone:'error', faIcon:'fa-triangle-exclamation', text:'Potential risk for active warrant', cta:'Contact us now', sub:'You may have missed a court date. Contact us before a warrant issues.' } },
    { tone:'info', text:'2 unread messages',             al:{ tone:'info', faIcon:'fa-comment-dots', text:'2 unread messages', cta:'View updates' } },
    { tone:'success', text:'Money-back issued · $240',   al:{ tone:'success', faIcon:'fa-circle-check', text:'Money-back issued', value:'$240', cta:'View refund', noFirm:true } },
  ]},
};
function PairTile({ badge, al }){
  ...
  const VIOLATION_BY_BADGE = { incomplete:'Not provided', active:'Speeding · VC 22349(a)', resolved:'Speeding · VC 22349(a)', dismissed:'Speeding · VC 22349', cancelled:'Speeding · VC 22349' };
  // The firm identity row is part of the standard card anatomy, so it rides in
  // the expanded tile for every matched state. Incomplete has no firm (booking
  // isn't finished, so no attorney has been matched yet).
  const FIRM_BY_BADGE = { active:{ firm:'Coastal Defense', rating:4.8 }, resolved:{ firm:'Mendez & Cole', rating:4.8 },
    dismissed:{ firm:'Okonkwo Legal', rating:5.0 }, cancelled:{ firm:'Harbor Legal', rating:4.7 } };
  const f = FIRM_BY_BADGE[badge];
  const c = { status:badge, city:'San Diego', mon:'Mar', year:'2026',
    ...
    violation: VIOLATION_BY_BADGE[badge] || 'Not provided', more:0,
    ...(f ? { firm:f.firm, attorney:{ name:f.firm, rating:f.rating } } : {}) };
  return (
    <div className="cpc" style={{ padding:13 }}>
      <CPCHeader c={c} />
      {c.attorney && !(al && (al.profile || al.noFirm)) && <div style={{ marginTop:13 }}><AttorneyRow c={c} /></div>}
      {al
        ? <GroupedAlert al={al} />
        : <div style={{ position:'absolute', right:13, bottom:12, display:'flex' }}><FIa name="chevron-right" size={16} color="var(--wf-faint)" /></div>}
    </div>
  );
}
...
function ProposedCPC(){
  return (
    <DCSection id="ba-pairing" title="Case Preview Cards" eyebrow="Component" updated="Updated • Tue Jul 7, 2026"
      ...
      <DCArtboard id="pair-active" label="Active" width={720} height={1960} style={{ background:'#fff' }}><PairingBoard badge="active" /></DCArtboard>
      <DCArtboard id="pair-resolved" label="Resolved" width={720} height={1040} style={{ background:'#fff' }}><PairingBoard badge="resolved" /></DCArtboard>
      <DCArtboard id="pair-dismissed" label="Dismissed" width={720} height={1320} style={{ background:'#fff' }}><PairingBoard badge="dismissed" /></DCArtboard>
      <DCArtboard id="pair-cancelled" label="Cancelled" width={720} height={1240} style={{ background:'#fff' }}><PairingBoard badge="cancelled" /></DCArtboard>
    </DCSection>
  );
}
