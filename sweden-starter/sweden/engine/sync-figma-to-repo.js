 ...
 *               When provided, syncs all modes. Light = $value. Dark = $extensions.com.sweden.modes.dark.
 *
...
function diffAndPatchJson(figmaVars, tokenMap, jsonData, figmaModes) {
  ...
  for (const mapping of tokenMap) {
    ...
    // ── Dark mode ($extensions.com.sweden.modes.dark) ──────────────────────
    ...
    if (varDarkId && figmaVar.valuesByMode && figmaVar.valuesByMode[varDarkId] && type === 'COLOR') {
      ...
      const currentDark   = token.$extensions?.['com.sweden.modes']?.dark;
        ...
        if (isDarkJsonAlias && isDarkFigmaAlias) {
            ...
            if (targetTokenKey) {
              ...
              if (targetTokenKey !== jsonDarkTarget) {
                ...
                fileEntry.data[tokenKey] = {
                  ...
                  $extensions: {
                    ...existing.$extensions,
                    'com.sweden.modes': { ...existing.$extensions?.['com.sweden.modes'], dark: newAlias },
                  },
                };
          ...
          if (darkDifferent) {
            ...
            fileEntry.data[tokenKey] = {
              ...
              $extensions: {
                ...existing.$extensions,
                'com.sweden.modes': { ...existing.$extensions?.['com.sweden.modes'], dark: darkFigmaHex },
              },
            };
...
async function run(figmaVars, tokenMap, figmaModes, opts = {}) {
  ...
  const prBody = [
    ...
    '',
    '---',
    '_Generated by Claude Code · Sweden DS sync script_',
  ].join('\n');
...
async function fetchAndRun(tokenMap, opts = {}) {
  ...
  console.log('\n── Sweden DS: Figma → JSON Sync (REST fetch) ─────────────────');
...
if (require.main === module) {
  ...
  if (varsArg) {
    ...
    console.log('\n── Sweden DS: Figma → JSON Sync (MCP vars) ──────────────────');
